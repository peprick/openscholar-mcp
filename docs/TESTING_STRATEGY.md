...
The optional live layer is manual and non-gating. Pull requests and clean-clone verification exercise the capture program only against loopback fake application and management servers, without contacting a scholarly provider or retaining a report. Against an isolated `EUROPE_PMC_ENABLED=true` backend with exactly OpenAlex and Europe PMC active, `node scripts/capture-europe-pmc-quality.mjs --base-url http://127.0.0.1:8080` runs the versioned `europe-pmc-live-queries-v1.json` query set through bounded first-page metadata searches and writes time-stamped evidence below ignored `backend/target/provider-quality/`. A split private management origin is selected with `--management-url`; it may use `OPENSCHOLAR_PROVIDER_QUALITY_METRICS_BEARER_TOKEN`. Production Actuator remains private and un-published, so reach it only through an approved loopback/private tunnel or evaluation endpoint—never a new public proxy route. The script requires exact two-provider coverage and exactly eight requests per provider and never requests PDFs, full text, supplements, or bulk data. Because it observes only the already fused page, it cannot measure isolated-provider/fused quality deltas or audit pre-reconciliation false merges. Default enablement therefore also requires a future bounded isolated-provider/raw-candidate evaluator, a disjoint independent holdout, and reviewed time-separated diagnostics. See [Provider quality evaluation](PROVIDER_QUALITY.md).
