...
`ProviderQualityComparativeRunSealBundleTests` pins the canonical v1 run-seal ID and SHA-256 vector, exact sorted payload paths, byte counts, deterministic layout, source non-mutation, POSIX-private published tree, exact verification, and exact-existing-bundle idempotence. It proves that one-byte and whitespace-only completed-worksheet changes produce different run-seal identities even when canonical judgments and score-report identity stay unchanged. It rejects missing, extra, symlinked, and tampered payloads; malformed, duplicate-key, trailing, noncanonical, or wrong-ID seals; wrong bindings or directory identity; missing or extra source-map entries; source symlinks; aggregate capture overflow; and republishing over a mismatched existing bundle without modifying that bundle.

After publication, the environment-gated runner reopens the promoted bytes and repeats strict capture verification and preflight, review-packet regeneration and exact verification, completed-worksheet compilation, exact canonical-judgment comparison, rescoring to the same result, exact score-report verification, and final run-seal verification before its path-free success record. The focused unit suite does not simulate concurrent ancestor/file replacement, force the filesystem-specific unsupported-atomic-move branch or a failure after staging begins, execute the complete environment-gated semantic replay, or capture stdout as a log allowlist test; do not treat those branches as independently tested.

This coverage proves deterministic local integrity only under the documented operator-controlled, non-concurrently-mutated filesystem assumption. It is not authentication, signing, trusted timestamping, WORM/immutable-storage, retention, access-history, or confidentiality evidence; those controls remain external. The promoted bundle is operator-only and must never become a reviewer artifact. There is no standalone no-write sealed-bundle replay mode.

The gated run uses no Spring context, Docker, network, PDF, UI, or runtime endpoint. Score generation writes a private ignored digest-bound report, report replay writes no new report, and run-seal promotion additionally writes the verified bundle to the explicitly configured external root. No real labels are checked in, no score is exposed to readers, and no policy threshold or default-enablement decision is implied. A clean live artifact still needs independent relevance and identity labels, a disjoint holdout, and time-separated reviewed diagnostics before any default-enablement decision. See [Provider quality evaluation](PROVIDER_QUALITY.md).
