...
The production frontend registers a small service worker that installs the account-neutral `/offline.html` fallback and dependency-free static reader runtime as one required, versioned cache pair. The active worker serves those exact reader assets cache-only, so an independent network refresh cannot mix shell and runtime versions; a later version replaces the pair only after both assets are fetched and written during a successful worker install. Manifest and icon requests remain network-first with a cached fallback, while queryless, same-origin `/_next/static/` build assets are cache-first and limited to 96 runtime entries. The allowlist excludes successful HTML/RSC responses, `/api`, direct REST, MCP, authentication, OAuth metadata, exports, cross-origin traffic, ranges, and research-document extensions. Successful eligible navigations remain network-only; an eligible navigation whose network fetch rejects receives the cached neutral fallback and reader. No search result, collection, session value, privacy export, or PDF byte enters CacheStorage.
