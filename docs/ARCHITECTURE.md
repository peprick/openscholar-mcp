...
The Next.js BFF treats even same-stack backend JSON as untrusted input. Successful JSON is streamed through an exact media-type, declared-length, actual-byte, fatal UTF-8, JSON, and Zod boundary with resource-specific ceilings: 8 MiB for 50-result search pages and one-paper details, 4 MiB for the 25-result related-paper view, 2 MiB for 100-item metadata-only collection/library pages, and 1 MiB for smaller responses. The encrypted offline-pack contract retains its separate exact 1 MiB ceiling. Problem Details uses a 64 KiB cap and falls back to a fixed safe problem on failure. Overflow cancels the upstream body. The personal-data export is intentionally not materialized by this reader: Spring Boot finishes its count and exact-byte preflight before sending headers, then the BFF validates the attachment contract and passes the stream and exact `Content-Length` through to the browser with `no-transform` disabling Next.js response compression.

## Browser installation and offline boundary

The production frontend registers a small service worker that installs the account-neutral `/offline.html` fallback and dependency-free static reader runtime as one required, versioned cache pair. Its registration URL, compiled worker, shell, runtime, declaration, and loader share one exact reader revision. The worker rejects an inexact script query or reuse of the active revision. Required assets are fetched without credentials and must be non-redirected, exact-URL, expected-type responses containing that revision; a failed fetch or cache write removes the whole candidate cache. The active worker serves the unqueried pair cache-only and maps only its own revision-qualified runtime request to it, while a newer revision bypasses the old worker. It does not force waiting workers to activate and deletes older OpenScholar caches only after safe activation. These rules keep the incumbent usable through failed, waiting, upgraded, and forward-rollback candidates without touching encrypted IndexedDB state.

Manifest and icon requests remain network-first with a cached fallback, while queryless, same-origin `/_next/static/` build assets are cache-first and limited to 96 runtime entries; these static/install fetches omit credentials. Cache admission rejects unsuccessful, opaque, redirected, exact-URL-mismatched, `private`, `no-store`, `Vary: *`, and `Vary: Cookie` responses. The allowlist excludes successful HTML/RSC responses, `/api`, direct REST, MCP, authentication, OAuth metadata, exports, cross-origin traffic, ranges, and research-document extensions. Successful eligible navigations remain network-only; an eligible navigation whose network fetch rejects receives the cached neutral fallback and reader. No search result, collection, session value, privacy export, or PDF byte enters CacheStorage.

A researcher may explicitly save one collection as a bounded, metadata-only offline pack. Spring Boot produces a current-owner, provider-free, no-store full snapshot; the browser validates it and atomically replaces the single `active` record in IndexedDB after encrypting it with a user-entered passphrase. The active record contains authenticated ciphertext plus versioned cryptographic parameters and opaque owner/collection mismatch locators. A separate non-secret `control` record binds the owner scope to a random lifecycle epoch. Save preparation binds that epoch before the snapshot request, and the final write rejects a changed epoch or a global/matching durable deletion intent. Beginning collection, account, or logout deletion atomically records that intent, rotates the epoch, and purges the affected local pack before the server mutation; a global intent supersedes a targeted one. IndexedDB schema version 2 preserves current records but rejects a legacy r2 writer after upgrade. The passphrase, derived key, and plaintext are memory-only. The static reader opens IndexedDB itself, requires the passphrase, renders untrusted metadata as text, and provides only local filtering, lock, and removal. The service worker never reads or decrypts the pack.

The pack is a best-effort copy, not a replica or backup. PostgreSQL remains authoritative, refresh is an explicit complete replacement, and there is no background synchronization, offline mutation, conflict resolution, access checking, provider request, or PDF reading. The browser may evict the record, a weak passphrase permits offline guessing, and same-origin script injection, extensions, or a compromised device can observe data while it is unlocked. A reachable hosted owner mismatch purges the pack before unlock. Collection/account deletion and hosted logout install a durable browser deletion fence before local purge and the server mutation; confirmed collection/account success removes it, while an ambiguous outcome remains fail-closed for a same-target retry. Collection/account deletion stops if that fence cannot be installed. Logout still proceeds in the exceptional storage-failure path so the app cannot trap the session. Successful hosted logout and account deletion also send `Clear-Site-Data: "storage"` as browser-dependent defense in depth. A disconnected browser or another device cannot be erased remotely.

The global connectivity notice treats `navigator.onLine` only as an advisory browser signal. When it reports offline, the UI makes a bounded, same-origin, `no-store`, credentialless request to `/api/connectivity`, which bypasses hosted session refresh and succeeds only when the Next.js route can reach an `UP` Spring Boot backend. AUTO and LOCAL submissions remain enabled during that initial advisory check and when the self-hosted/local stack is reachable; the UI instead warns that online research sources may be limited. Confirmed application unreachability disables those server-backed search actions. **Check again** or a returning browser signal starts a recovery probe after transient failure, but actions resume and assistive technology receives a recovery announcement only after that probe succeeds. UI copy reports application reachability rather than asserting Internet state. The probe does not infer individual scholarly-provider health, cache its result, or queue a mutation for later replay. See [ADR 0007](decisions/0007-use-an-account-neutral-pwa-shell.md) and [ADR 0010](decisions/0010-store-one-encrypted-offline-metadata-pack.md).
...
The production policy remains exact-only. A strict, SHA-256-bound `paper-deduplication-development-v2.json` now exercises the real catalog path across three frozen ingest orders, with a separate machine-readable evaluation policy and activation gates. It records metadata-only positives and must-separate boundaries but deliberately does not implement or enable a metadata fallback. A disjoint independently authored holdout is still required before step 4 can be considered for activation.
...
- Personal-data export first acquires fail-fast instance-global and current-principal permits before a transaction or response target opens. Defaults are four exports per backend instance and one per authenticated principal. Accepted exports use ordered JDBC cursor visitors inside one read-only `REPEATABLE READ` transaction. A bounded count preflight followed by an exact null-sink serialization rejects more than 100,000 combined search/collection/saved-paper rows or more than 128 MiB of UTF-8 JSON before the servlet response opens. The same snapshot is then serialized a second time directly to the response; the configured database transaction/JDBC timeout is 120 seconds, and the measured byte count becomes the exact `Content-Length`. The global gate is not cluster-wide, so replicas multiply aggregate capacity and need a separate edge or distributed control when required.
...
The server registers and advertises exactly three read-only JSON templates: `openscholar://papers/{paperId}`, `openscholar://collections/{collectionId}`, and `openscholar://searches/{searchId}`. Template discovery exposes only those parameterized URIs, and a resource read delegates to bounded database-backed application reads rather than persistence entities. Paper IDs, collection IDs, and search IDs must be canonical UUIDs; SDK routing plus the OpenScholar URI parser rejects alternate schemes, extra segments, queries, fragments, encoded URI variants, and malformed identifiers before dispatch. Matched-template errors are fixed and identifier-free; the SDK's unmatched-resource protocol error can reflect only the caller-supplied URI.

Paper resources contain bounded stored canonical metadata. Collection and immutable-search resources resolve the authenticated MCP owner and return only owner-visible, bounded JSON views; missing and foreign-owner identifiers are deliberately indistinguishable. The resource adapter does not invoke discovery or access-provider adapters, dereference arbitrary URLs, read filesystem paths, or return PDF/source bytes. `resources/list` returns an empty concrete list, subscriptions and resource-change notifications are not implemented, and initialization leaves `subscribe` and `listChanged` unadvertised. Unit, raw-wire, and hosted-owner integration tests cover these boundaries.
...
- The synchronous privacy-export path has a coordinated timeout envelope: a 120-second backend transaction/query timeout, 140 seconds for the frontend to receive backend response headers, 30 seconds to consume a non-success backend body, 150 seconds for Caddy to receive privacy-export response headers, and a 340-second Caddy request-to-final-byte write ceiling. The transaction timeout configures JDBC work but cannot interrupt a blocked servlet output write. The frontend clears its header timer before an accepted body streams, requests identity encoding, and rejects encoded upstream responses rather than applying a stale compressed length to decompressed bytes. Generic proxy routes retain their shorter response-header timeout. Caddy's write timeout starts after it reads the request headers and is listener-wide rather than privacy-route-specific; after the privacy route's five-second dial and full 150-second upstream-header allowances, 340 seconds budgets roughly 180 seconds for the body plus a small handler margin. The longer listener-wide value is an explicit availability tradeoff needed by this synchronous path. Caddy does not compress either privacy-export path, preserving the exact preflighted representation length at the public edge.
