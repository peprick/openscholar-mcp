...
Those values are engineering evidence and never appear in the reader UI, REST API, or MCP tools. Synthetic fixtures can detect mechanics regressions but cannot prove current live usefulness. An optional capture samples bounded, already fused first-page metadata into an ignored local report; it cannot compute isolated OpenAlex/Europe-PMC/fused quality deltas or audit raw pre-reconciliation false merges. Europe PMC remains disabled by default, and any future default change requires a bounded isolated-provider/raw-candidate evaluator, a disjoint independent holdout, and multiple time-separated, reviewed live diagnostics. The fixtures, commands, private management boundary, and advancement rule are documented in [Provider quality evaluation](PROVIDER_QUALITY.md).
