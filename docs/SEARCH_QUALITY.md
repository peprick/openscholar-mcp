...
Judgment release is atomic and one-shot only for a particular `VerifiedCorpus` instance. A fresh `verifyCorpus` call can stage the same files again, so a durable operator ledger is still required to enforce first-run finality across intake calls and processes. These utilities prove ranking and scoring mechanics only for synthetic metadata; the coordinator capability by itself cannot authenticate which callback ran. A pure source-seal boundary binds revision/state facts and exact source bytes, while a canonical clean-checkout Git collector gathers those facts from a standalone clone and requires independently retained evaluator/candidate digests. A canonical four-artifact report plus exact-byte verifier binds that seal to the complete snapshot and score without authorizing reader metrics, external acceptance, custody, or activation. A private read-only retained-bundle verifier can compare those opaque bytes with a canonical external manifest under an operator-controlled filesystem assumption. No real bundle or reviewer label is checked in, and there is still no durable ledger, operator command, frozen evaluator revision/source digest, or safe publisher. Portable Java NIO cannot guarantee atomic directory installation without replacing a concurrently created target, so publication is explicitly deferred to a reviewed native exclusive-rename boundary. External acceptance and custody release remain unauthorized until the complete evaluator revision is independently frozen and a controlled operator builds and runs that same verified checkout. Digest validation, in-process types, and declarations do not prove reviewer independence, process isolation, tool authenticity, or semantic disjointness.
