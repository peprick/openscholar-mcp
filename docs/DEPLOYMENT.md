...
The frontend serves `/sw.js` with `Cache-Control: no-cache, no-store, must-revalidate` and `Service-Worker-Allowed: /`. Verify those headers, network-first manifest/icon updates, and the account-neutral offline fallback through the public origin after each frontend release. Do not add an edge cache rule for the worker. The worker retains at most 96 queryless same-origin `/_next/static/` runtime entries, refreshes the neutral fallback after successful eligible navigation, and never stores successful application pages, APIs, authentication, exports, MCP traffic, or documents.
