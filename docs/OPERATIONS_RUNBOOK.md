...
This runbook covers the single-host deployment template. It does not replace a cloud-provider recovery procedure or an organization incident-response plan. Commands assume an operator has selected the intended host and uses the reviewed `deploy/production.env` file.

Set a shell alias or wrapper for the exact deployment command to avoid selecting a development project accidentally:
...
Always render `config --quiet` before an update. Never paste secret values into tickets or log commands.
...
- Review sanitized provider error/rate-limit logs without credentials or private queries. The current blackbox profile does not export application-internal provider/job/MCP meters.
...
docker compose --env-file deploy/production.env \
  -f deploy/compose.production.yaml ps
docker compose --env-file deploy/production.env \
  -f deploy/compose.production.yaml logs --since 30m --no-color backend frontend proxy
docker compose --env-file deploy/production.env \
...
2. Check DNS/TLS from an independent network, then `compose ps` and bounded recent logs.
...
```bash
docker compose --env-file deploy/production.env \
  -f deploy/compose.production.yaml stop backend frontend
...
6. Start backend, wait for readiness, start frontend/proxy, and complete smoke checks.
