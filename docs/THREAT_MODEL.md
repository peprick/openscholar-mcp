...
5. An MCP template read crosses the same authenticated boundary into one bounded database-only paper, collection, or saved-search view. Collection and search ownership is resolved from the validated principal; no URI supplied through that surface becomes an outbound or filesystem target.
6. An explicit offline-pack request returns one bounded, owner-scoped, provider-free metadata snapshot through a no-store BFF route. The browser encrypts it before one atomic IndexedDB replacement; the passphrase and derived key do not cross the network or persist intentionally.
7. An explicit privacy export first passes fail-fast backend-instance-global and authenticated-principal admission, then reads only that owner's rows through one repeatable-read database snapshot. Spring Boot count- and byte-preflights it before committing a bounded, no-store attachment; Next.js and Caddy pass the accepted response stream through without retaining it.
8. A user-selected verified PDF URL is fetched directly by the isolated browser reader from the external document host.
9. CI obtains source and dependencies, builds artifacts, generates an SBOM, and publishes scan findings. This boundary can affect every runtime asset.
10. Operators and backup/monitoring systems cross a privileged administrative boundary not exposed through Caddy.
...
| Offline-pack disclosure, stale retention, or cross-account display | Explicit one-pack opt-in; current-owner, no-store, provider-free full snapshot; 500-paper/1 MiB bounds; PBKDF2-HMAC-SHA-256 at 600,000 iterations; fresh salt/IV; authenticated AES-256-GCM; non-extractable memory-only key; opaque owner mismatch check; save preparation binds owner/collection/epoch before snapshot fetch; a durable global or collection deletion intent is installed atomically with epoch rotation and local purge; global intent supersedes targeted intent; IndexedDB v2 rejects the legacy r2 writer; the final transaction rejects changed fences or matching deletion; successful logout/account-deletion `Clear-Site-Data` | Weak passphrases permit offline guessing; XSS, extensions, screen capture, and a compromised device can observe unlocked data; browser deletion is not forensic erasure; an ambiguous deletion intentionally leaves saving fail-closed until a same-target retry; direct storage/API use does not participate in the browser protocol; a logout whose browser storage is unavailable relies on response clearing and later owner mismatch; quota eviction, crashes, suspended tabs, and other devices prevent a universal purge guarantee |
| Cross-user data access / IDOR | Search snapshots and libraries use issuer+subject-derived ownership; local-catalog candidates require a paper to be visible through that owner's prior snapshot or saved collection; other-owner identifiers return not found; search-refresh visibility follows snapshot ownership; privacy export/delete are current-principal operations; export count, dry run, and final cursor pass share one owner-scoped repeatable-read snapshot; negative authorization tests exist | Shared `PAPER_ACCESS` jobs are visible/retryable to every `openscholar.jobs` principal; shared canonical/access records remain by design; review every new owned resource and local ranking join |
...
| MCP resource enumeration, IDOR, or URI-driven fetch | Discovery is limited to exactly three read-only templates; strict canonical UUIDs, bounded JSON, owner-safe collection/search reads, database-only dispatch, an empty concrete resource list, and no subscription/change-notification capability are implemented and tested | Preserve malformed-URI, owner-equivalence, provider-isolation, and output-limit regression tests; review every added template or capability |
...
| Denial of service / quota exhaustion | Payload/body/time budgets, bounded concurrent fan-out/results, partial failure, cache, cooldown, per-principal MCP limits, internal database network; privacy export has fail-fast instance-global/per-principal admission before transaction start, a 100,000-row/128 MiB pre-commit ceiling, cursor fetch bounds, a 120-second database timeout, a finite error-body abort, and coordinated frontend/edge header timeouts | The database timeout cannot interrupt a blocked response write; privacy export deliberately performs two serializations in one long-lived snapshot; the admission budget is per JVM and replicas multiply it; Caddy's 340-second request-to-final-byte timeout applies listener-wide; add aggregate load evidence, cluster/upstream connection limits, quota alerts, slow-client testing, and abuse policy |
...
- Missing versus other-owner collection/search resource reads, including timing, JSON-RPC error shape, and observable metadata.
- MCP resource URIs with non-canonical or malformed IDs, alternate schemes, extra segments, queries, fragments, encoded separators, or unknown templates.
- Attempts to enumerate all resources, subscribe or unsubscribe, solicit resource-change notifications, exceed the collection-page or serialized-JSON ceiling, retrieve PDF bytes, invoke a provider, or turn a resource identifier into an arbitrary URL/filesystem read.
...
- Oversized or cross-owner offline-pack requests; modified algorithms/work factors/salts/IVs/ciphertext; wrong passphrases; quota failure during replacement; owner mismatch; delayed/fresh cross-tab saves during deletion; and response-lost deletion with an offline pack present.
...
- No MCP resource read invokes a provider, dereferences a caller-controlled URL, reads a filesystem path, returns PDF/source bytes, or bypasses owner checks and fixed output ceilings.
- The advertised MCP resource capability is limited to the three-template discovery/read contract; the concrete resource list stays empty, and subscriptions and change notifications remain outside that contract.
...
Account deletion removes the current principal's searches, search-refresh jobs, collections, memberships/tags, and hosted internal user row. Before the request, the browser performing the operation installs a global deletion intent, rotates the lifecycle fence, and purges its offline pack; an unconfirmed result leaves saving fail-closed. A successful response completes the intent and uses `Clear-Site-Data: "storage"` as defense in depth. Neither mechanism guarantees erasure from a disconnected or different browser profile/device. Deletion does not remove shared canonical/provider/access records or global access-refresh jobs, and it does not revoke the account at the identity provider. A later valid token for the same issuer+subject provisions a new empty internal user.

## Review triggers

Revisit this model when changing the identity provider/authentication design, adding a provider, adding an MCP resource outside the three-template contract or enabling listing/subscription semantics, changing the offline-pack payload/envelope/KDF, supporting multiple packs or offline writes, changing the service-worker allowlist, adding document retention/upload, LLM summarization, a write-capable MCP tool, collaboration/tenancy, object storage, a new egress path, a worker service, public metrics, or a new deployment environment. Record the reviewer, date, changed boundaries, tests, accepted residual risks, and expiry of each temporary exception.
