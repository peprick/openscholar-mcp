...
7. A user-selected verified PDF URL is fetched directly by the isolated browser reader from the external document host.
8. CI obtains source and dependencies, builds artifacts, generates an SBOM, and publishes scan findings. This boundary can affect every runtime asset.
9. Operators and backup/monitoring systems cross a privileged administrative boundary not exposed through Caddy.
