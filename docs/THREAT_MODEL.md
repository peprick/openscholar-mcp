...
| Cross-user data access / IDOR | Search snapshots and libraries use issuer+subject-derived ownership; other-owner identifiers return not found; search-refresh visibility follows snapshot ownership; privacy export/delete are current-principal operations; negative authorization tests exist | Shared `PAPER_ACCESS` jobs are visible/retryable to every `openscholar.jobs` principal; shared canonical/access records remain by design; review every new owned resource |
...
| Query/privacy leakage to providers | Backend chooses bounded metadata queries; provider payload retention is minimized; owner-scoped no-store export and confirmed account deletion are implemented | Approve privacy notice, purpose/retention periods, jurisdiction/vendor review, backup deletion treatment, and shared-catalog policy; avoid sending private notes/full text |
...
| Supply-chain compromise | Lockfiles, checksum-verified Maven Wrapper, reviewed immutable action/image pins with a drift validator, non-root multi-stage images, Dependabot, dependency review, CodeQL, Trivy SARIF and CycloneDX SBOM workflow | Sign/attest release-built application images, protect environments, define exceptions/patch SLA |
| Container escape / lateral movement | Read-only root filesystems, dropped capabilities where compatible, `no-new-privileges`, non-root app/proxy/monitoring, isolated internal networks | Host/runtime patching, seccomp/AppArmor policy, rootless-runtime feasibility, external penetration review |
