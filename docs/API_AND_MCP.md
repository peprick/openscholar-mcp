...
OpenAlex is enabled by default. Europe PMC, DataCite, DOAJ, and CORE are operator opt-ins; CORE additionally requires the licence-confirmation guard. The five discovery adapters run through a bounded concurrent fan-out whose configured concurrency defaults to five. A successful provider is preserved when another fails, coverage/warnings describe both outcomes, exact identifiers merge duplicate works, and a multi-provider page is ranked by deterministic reciprocal-rank fusion with provider contributions retained. DataCite is thesis/dissertation metadata-only discovery and never returns a discovery PDF or open-access claim.

Europe PMC is default-off, metadata-only discovery. It calls only the Articles REST `/search` route, restricts results to `SRC:MED` journal articles held in PMC, and maps DOI, PMID, PMCID, abstracts, and bounded bibliographic metadata. It ignores `fullTextUrlList`, never calls `fullTextXML`, supplementary-file, PDF, or bulk-download routes, and always returns a null discovery `pdfUrl`. A returned open-access value is an unverified provider hint whose rights vary by article; it is not a verified location or reuse grant.
...
OIDC mode requires `openscholar.privacy`. Export returns a no-store, nosniff JSON attachment containing only the current principal's identity display data, search snapshots/filters, collections, and saved-paper memberships. A fail-fast per-instance global and current-principal gate runs before any export transaction or attachment opens; saturation returns safe `429 PRIVACY_EXPORT_BUSY` Problem Details with `Retry-After`. One read-only `REPEATABLE READ` transaction holds an accepted export's snapshot while ordered JDBC cursors perform a bounded count preflight, an exact UTF-8 dry serialization, and the final serialization. The combined `searches`, `collections`, and `savedPapers` arrays may contain at most 100,000 rows and the whole serialized document may contain at most 134,217,728 bytes. The target opens only after both preflights pass, the accepted response carries the exact `Content-Length`, and an oversized export returns `422 PRIVACY_EXPORT_TOO_LARGE` rather than a partial document. The configured database transaction/JDBC timeout is 120 seconds.

Account deletion requires the exact `DELETE_MY_DATA` confirmation and deletes that principal's search snapshots, search-refresh jobs, collections/memberships/tags, and OIDC user row. Shared canonical paper/provider/access metadata and global access-refresh jobs are not personal-account records and are not deleted. A later valid token for the same issuer+subject provisions a new empty internal account. Local mode preserves the fixed bootstrap user while deleting its personal search/library data.

The web privacy center at `/data` exposes both controls through same-origin Next.js routes. `GET /api/privacy/export` requests an identity-encoded backend representation, rejects any encoded response, validates the attachment media type and bounded exact length, forwards the authenticated request server-side, and passes the body through without `blob()`/object-URL buffering. It returns only allowlisted attachment headers with `Cache-Control: no-store, no-transform`, which disables Next.js response compression and preserves the measured representation length. Its 140-second response-header timer is cleared as soon as backend headers arrive, so it cannot truncate an accepted stream; a non-success response body instead receives a separate 30-second abort deadline. The header timer is coordinated with the backend's 120-second transaction/query timeout and Caddy's privacy-specific 150-second response-header timeout. The database timeout is not an end-to-end bound on a blocked servlet write. Generic proxy response-header timeouts remain unchanged. `DELETE /api/privacy/account` accepts only the exact confirmation object, forwards no additional client fields, and clears the OpenScholar hosted-session cookie after a successful deletion. These browser routes do not expose issuer/subject identifiers, shared catalog records, PDFs, provider diagnostics, refresh jobs, or operational metrics.
