...
- Add an independently authored blind related-topic reuse holdout and target-deployment performance evidence before considering any product path. The current candidate was tuned against visible synthetic `DEVELOPMENT` labels, then frozen for regression; despite measured development gains, its expansion still surfaced owner-visible off-topic controls below rank one and is not activation evidence. The [holdout protocol](RELATED_TOPIC_REUSE_HOLDOUT_PROTOCOL.md) freezes a future input schema, metric semantics, decision gates, and fail-closed test-only parser, but includes no real holdout, evaluator, operator command, or result; external acceptance remains unauthorized until the evaluator implementation is frozen. The frozen [100,000-paper scale protocol](RELATED_TOPIC_REUSE_PERFORMANCE_PROTOCOL.md) is a single-caller, warm-cache Testcontainers diagnostic with record-only latency and does not satisfy either requirement.
