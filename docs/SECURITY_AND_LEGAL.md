...
- The `/data` web privacy center calls those APIs only through same-origin, no-store Next.js routes. The export route forwards only safe attachment headers; the deletion route validates the exact confirmation shape before backend access and expires the OpenScholar hosted-session cookie only after successful deletion. The normal UI attempts to purge and lock the local offline pack before destructive account/collection operations or hosted logout. Successful account deletion and hosted logout also return `Clear-Site-Data: "storage"` as browser-dependent defense in depth. Because a response can be lost after a committed delete, an unconfirmed browser result never claims that server data was preserved and directs the user to refresh before retrying. No privacy route places personal data in CacheStorage, and no browser mechanism can synchronously erase a disconnected copy on another device.
