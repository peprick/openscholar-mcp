...
The production service worker caches only an account-neutral static fallback and offline reader runtime, the manifest/icons, and queryless same-origin `/_next/static/` build assets. The fallback and reader runtime are a required, versioned pair: installation completes only after both are fetched and written, and the active worker serves their exact paths cache-only until a later worker version installs a complete replacement. This prevents an independent network refresh from mixing reader versions. Manifest and icon requests are network-first with a cached fallback; runtime `/_next/static/` entries are cache-first and capped at 96. Successful eligible navigations remain network-only and receive the cached fallback only when their network fetch rejects; neither their page/RSC response nor a background shell refresh is stored. APIs, authentication, MCP/OAuth traffic, exports, authorization/range requests, cross-origin responses, and research documents are excluded. Unsuccessful, opaque, `private`, `no-store`, and `Vary: *` responses are rejected before storage. This positive allowlist and bound are the CacheStorage security boundary; response cache headers are defense in depth.
