 ...
 */
final class DataCiteRequestDeadline implements ClientHttpRequestInterceptor {

	private final long timeoutNanos;
	private final LongSupplier ticker;

	DataCiteRequestDeadline(Duration timeout) {
		this(timeout, System::nanoTime);
	}

	DataCiteRequestDeadline(Duration timeout, LongSupplier ticker) {
		if (timeout == null || timeout.isZero() || timeout.isNegative()) {
			throw new IllegalArgumentException("DataCite request deadline must be positive");
		}
	...
	static boolean wasExceeded(Throwable failure) {
		for (Throwable current = failure; current != null; current = current.getCause()) {
			if (current instanceof DataCiteRequestDeadlineExceededException) {
				return true;
			}
		}
		return false;
	...
	private static long effectiveTimeoutNanos(Duration timeout) {
		...
		if (timeoutMillis < 1) {
			throw new IllegalArgumentException("DataCite request deadline must be at least one millisecond");
		}
	...
	private record Deadline(long startedAtNanos, long timeoutNanos, LongSupplier ticker) {
		...
		private IOException markIfExceeded(IOException failure) {
			if (failure instanceof DataCiteRequestDeadlineExceededException || !isExceeded()) {
				return failure;
			}
			return new DataCiteRequestDeadlineExceededException(failure);
		}

		private void throwIfExceeded() throws DataCiteRequestDeadlineExceededException {
			if (isExceeded()) {
				throw new DataCiteRequestDeadlineExceededException();
			}
		}
	...
	private static final class DataCiteRequestDeadlineExceededException extends IOException {

		private DataCiteRequestDeadlineExceededException() {
			super("DataCite request deadline exceeded");
		}

		private DataCiteRequestDeadlineExceededException(IOException cause) {
			super("DataCite request deadline exceeded", cause);
		}
	}
}
