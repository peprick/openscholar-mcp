 ...
 * partially consumed response body at the configured whole-exchange deadline.
 */
final class CoreRequestDeadline implements ClientHttpRequestInterceptor {

	private final long timeoutNanos;
	private final LongSupplier ticker;

	CoreRequestDeadline(Duration timeout) {
		this(timeout, System::nanoTime);
	}

	CoreRequestDeadline(Duration timeout, LongSupplier ticker) {
		if (timeout == null || timeout.isZero() || timeout.isNegative()) {
			throw new IllegalArgumentException("CORE request deadline must be positive");
		}
	...
	static boolean wasExceeded(Throwable failure) {
		for (Throwable current = failure; current != null; current = current.getCause()) {
			if (current instanceof CoreRequestDeadlineExceededException) {
				return true;
			}
		}
		return false;
	...
	private static long effectiveTimeoutNanos(Duration timeout) {
		...
		if (timeoutMillis < 1) {
			throw new IllegalArgumentException("CORE request deadline must be at least one millisecond");
		}
	...
	private record Deadline(long startedAtNanos, long timeoutNanos, LongSupplier ticker) {
		...
		private IOException markIfExceeded(IOException failure) {
			if (failure instanceof CoreRequestDeadlineExceededException || !isExceeded()) {
				return failure;
			}
			return new CoreRequestDeadlineExceededException(failure);
		}

		private void throwIfExceeded() throws CoreRequestDeadlineExceededException {
			if (isExceeded()) {
				throw new CoreRequestDeadlineExceededException();
			}
		}
	...
	private static final class CoreRequestDeadlineExceededException extends IOException {

		private CoreRequestDeadlineExceededException() {
			super("CORE request deadline exceeded");
		}

		private CoreRequestDeadlineExceededException(IOException cause) {
			super("CORE request deadline exceeded", cause);
		}
	}
}
