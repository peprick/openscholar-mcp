 ...
 */
final class OpenAlexRequestDeadline implements ClientHttpRequestInterceptor {

	private final long timeoutNanos;
	private final LongSupplier ticker;

	OpenAlexRequestDeadline(Duration timeout) {
		this(timeout, System::nanoTime);
	}

	OpenAlexRequestDeadline(Duration timeout, LongSupplier ticker) {
		if (timeout == null || timeout.isZero() || timeout.isNegative()) {
			throw new IllegalArgumentException("OpenAlex request deadline must be positive");
		}
	...
	static boolean wasExceeded(Throwable failure) {
		for (Throwable current = failure; current != null; current = current.getCause()) {
			if (current instanceof OpenAlexRequestDeadlineExceededException) {
				return true;
			}
		}
		return false;
	...
	private static long effectiveTimeoutNanos(Duration timeout) {
		...
		if (timeoutMillis < 1) {
			throw new IllegalArgumentException("OpenAlex request deadline must be at least one millisecond");
		}
	...
	private record Deadline(long startedAtNanos, long timeoutNanos, LongSupplier ticker) {
		...
		private IOException markIfExceeded(IOException failure) {
			if (failure instanceof OpenAlexRequestDeadlineExceededException || !isExceeded()) {
				return failure;
			}
			return new OpenAlexRequestDeadlineExceededException(failure);
		}

		private void throwIfExceeded() throws OpenAlexRequestDeadlineExceededException {
			if (isExceeded()) {
				throw new OpenAlexRequestDeadlineExceededException();
			}
		}
	...
	private static final class OpenAlexRequestDeadlineExceededException extends IOException {

		private OpenAlexRequestDeadlineExceededException() {
			super("OpenAlex request deadline exceeded");
		}

		private OpenAlexRequestDeadlineExceededException(IOException cause) {
			super("OpenAlex request deadline exceeded", cause);
		}
	}
}
