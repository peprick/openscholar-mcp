 ...
 */
final class DoajRequestDeadline implements ClientHttpRequestInterceptor {

	private final long timeoutNanos;
	private final LongSupplier ticker;

	DoajRequestDeadline(Duration timeout) {
		this(timeout, System::nanoTime);
	}

	DoajRequestDeadline(Duration timeout, LongSupplier ticker) {
		if (timeout == null || timeout.isZero() || timeout.isNegative()) {
			throw new IllegalArgumentException("DOAJ request deadline must be positive");
		}
	...
	static boolean wasExceeded(Throwable failure) {
		for (Throwable current = failure; current != null; current = current.getCause()) {
			if (current instanceof DoajRequestDeadlineExceededException) {
				return true;
			}
		}
		return false;
	...
	private static long effectiveTimeoutNanos(Duration timeout) {
		...
		if (timeoutMillis < 1) {
			throw new IllegalArgumentException("DOAJ request deadline must be at least one millisecond");
		}
	...
	private record Deadline(long startedAtNanos, long timeoutNanos, LongSupplier ticker) {
		...
		private IOException markIfExceeded(IOException failure) {
			if (failure instanceof DoajRequestDeadlineExceededException || !isExceeded()) {
				return failure;
			}
			return new DoajRequestDeadlineExceededException(failure);
		}

		private void throwIfExceeded() throws DoajRequestDeadlineExceededException {
			if (isExceeded()) {
				throw new DoajRequestDeadlineExceededException();
			}
		}
	...
	private static final class DoajRequestDeadlineExceededException extends IOException {

		private DoajRequestDeadlineExceededException() {
			super("DOAJ request deadline exceeded");
		}

		private DoajRequestDeadlineExceededException(IOException cause) {
			super("DOAJ request deadline exceeded", cause);
		}
	}
}
